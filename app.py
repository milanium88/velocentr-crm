...
def display_inventory():
    ...
    with st.expander("⚙️ Дополнительные фильтры", expanded=False):
    ...
    excel_download(export_df, "velocentr_sklad.xlsx", "⬇️ Экспорт товаров в Excel")


def product_editor():
    st.markdown("---")
    st.header("⚙️ Управление товарами")
    ...
    if mode == "Добавить":
        ...
        with st.form(f"edit_product_{pid}"):
            st.subheader(f"✏️ Редактирование: {p.get('name', '')}")
...
def crm_clients_section():
    ...
    for c in clients:
        ...
        with st.container():
            ...
            with col3:
                if st.button("✏️", key=f"edit_c_{cid}", help="Редактировать"):
    ...
    for i, status in enumerate(kanban_statuses):
        with cols[i]:
            ...
            for t in status_tasks:
                ...
                with btn_cols[0]:
                    if st.button("✏️", key=f"kb_edit_{tid}", help="Редактировать"):
...
def task_edit_form(edit_id):
    ...
    if edit_id == "new":
        ...
        if not t:
            ...
            return
        st.subheader(f"✏️ Задача #{edit_id}")
...
def crm_calls_section():
    ...
    for x in sorted(calls, key=lambda r: str(r.get("date", "")), reverse=True):
        ...
        with st.container():
            ...
            with col2:
                if st.button("✏️", key=f"edit_x_{x.get('id')}", help="Редактировать"):
...
def call_edit_form(edit_id):
    ...
    if edit_id == "new":
        ...
        if not x:
            ...
            return
        st.subheader(f"✏️ Звонок #{edit_id}")
...
def categories_editor():
    ...
    if col2.button("⬆️ Вверх", disabled=(index == 0)):
        cats[index], cats[index - 1] = cats[index - 1], cats[index]
        save_data()
        st.rerun()
    if col3.button("⬇️ Вниз", disabled=(index == len(cats) - 1)):
        cats[index], cats[index + 1] = cats[index + 1], cats[index]
        save_data()
        st.rerun()
...
def render_notifications():
    """Уведомления — одна строка, кликабельная."""
    today = datetime.now().date()
    ...
    if not urgent:
        return
    st.warning(f"🔔 {len(urgent)} задач требуют внимания — раздел CRM", icon="⚠️")
...
def render_sidebar():
    ...
    menu = st.sidebar.radio(
        "Разделы",
        ("📊 Дашборд", "📦 Склад / Товары", "👥 CRM", "🏷 Категории", "⚙️ Настройки"),
        key="main_menu",
    )
